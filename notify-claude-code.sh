...
log_debug() {
    ...
    local logfile="${CLAUDE_NOTIFY_DEBUG_LOG:-}"
    if [ -z "$logfile" ]; then
        local home="${HOME:-}"
        # Fail open: hooks may run without HOME; skip logging instead of crashing.
        [ -n "$home" ] || return 0
        logfile="$home/.claude/log/notify-claude-code.log"
    fi
...
case "$EVENT_NAME:$NOTIF_TYPE" in
    PostToolUseFailure:*|Notification:permission_prompt|Notification:idle_prompt|Notification:elicitation_dialog) NOTIFY_KIND="attention" ;;
esac
...
bell_arg="$(tmux_notify_agent_bell_arg CLAUDE)"
if [ -n "$bell_arg" ]; then
    args+=("$bell_arg")
fi
